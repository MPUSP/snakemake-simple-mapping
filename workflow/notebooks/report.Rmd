...
- the following figure contains the same information as in the previous figures,
...
```{r, echo = FALSE, warning = FALSE, fig.width = figwidth, fig.height = 2.2}
df_plotting <- df_filtered %>%
  filter(VARTYPE == "stop_lost") %>%
  group_by(CHROM, POS, REF, ALT, VARTYPE, IMPACT, GENE, AA_CHANGE, CODON_CHANGE, INDEL) %>%
  summarize(samples = list(FILE))

for (i in 1:nrow(df_plotting)) {
  plot_genome_stop_lost <- plot_genome(df_tx, df_plotting[i, ])
  print(plot_genome_stop_lost)
  save_plot(plot_genome_stop_lost, height = 2.0, index = i)
}
```
...
for (i in 1:nrow(df_plotting)) {
  ...
  save_plot(plot_genome_stop_gained, height = 2.0, index = i)
}
```
...
```{r, echo = FALSE, warning = FALSE, fig.width = figwidth, fig.height = 2.2}
df_plotting <- df_filtered %>%
  filter(VARTYPE == "frameshift_variant") %>%
  group_by(CHROM, POS, REF, ALT, VARTYPE, IMPACT, GENE, AA_CHANGE, CODON_CHANGE, INDEL) %>%
  summarize(samples = list(FILE))
...
for (i in 1:nrow(df_plotting)) {
  ...
  save_plot(plot_genome_frameshift_variant, height = 2.0, index = i)
}
```
...
```{r, echo = FALSE, warning = FALSE, fig.width = figwidth, fig.height = 2.2}
df_plotting <- df_filtered %>%
  filter(VARTYPE == "missense_variant") %>%
  group_by(CHROM, POS, REF, ALT, VARTYPE, IMPACT, GENE, AA_CHANGE, CODON_CHANGE, INDEL) %>%
  summarize(samples = list(FILE))
...
for (i in 1:nrow(df_plotting)) {
  ...
  save_plot(plot_genome_missense_variant, height = 2.0, index = i)
}
```
